    ...
    switch (columnId) {
        case "status":
            if (value === "Activo" || value === "Inactivo")
                updateRow(rowId, { status: value });
            return;
        case "department":
            if (typeof value === "string")
                updateRow(rowId, { department: value as Row["department"] });
            return;
        case "name":
        ...
        case "createdAt":
            if (typeof value === "string")
                updateRow(rowId, { createdAt: value });
            return;
    }
};
...
- Accesibilidad: los encabezados de columna que permiten ordenar ahora son keyboard-accessible (`role='button'`, `tabindex`, manejadores `@keydown`) y exponen `aria-sort` para lectores de pantalla. ✅
- Filas clicables: ahora son focusables (`tabindex=0`) y soportan activar con Enter (delegan en el mismo manejador que el click). Las interacciones internas (botones, inputs) siguen evitando el `rowClick`.
- Rendimiento/claridad: se añadieron `headerGroups` y `leafColumns` como `computed` para evitar llamadas repetidas a `table.getHeaderGroups()` y `table.getAllLeafColumns()` y hacer la plantilla más legible.
- Filtro de fechas: `getDateRangeForPicker` devuelve ahora `Date | [Date, Date|null] | null` de forma segura (sin casts inseguros) y `TableFiltersAdvanced.vue` fue actualizado para reflejar este comportamiento.
- Typescript: se ajustó el uso de `role` para usar `undefined` en lugar de `null`, corrigiendo un error de tipo de compilación.
- Recomendación: considerar extraer la UI de edición inline a un componente `EditableCell.vue` para simplificar `TanStackTable.vue` y facilitar pruebas.
