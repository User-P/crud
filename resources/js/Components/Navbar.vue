<template>
    ...
    <div v-if="mobileSearchOpen" class="nav fixed inset-x-0 top-0 z-50 flex h-14 items-center gap-3 px-4">
        <Icon icon="heroicons:magnifying-glass" class="crumb-separator h-4 w-4 shrink-0" aria-hidden="true" />
        <input ref="mobileSearchInput" type="search" placeholder="Buscar..."
            class="search-input min-w-0 flex-1 rounded-xl py-2 px-3 text-sm placeholder:italic focus:ring-0"
            @keydown.escape="mobileSearchOpen = false" />
        <button type="button" class="btn rounded-xl p-2" aria-label="Cerrar búsqueda"
    ...
    <header
        class="nav sticky top-0 z-40 flex h-14 shrink-0 items-center justify-between gap-2 px-3 sm:gap-4 sm:px-6 lg:px-8"
        :class="{ 'invisible': mobileSearchOpen }">
        <!-- Izquierda: menú + logo -->
        <div class="flex min-w-0 shrink-0 items-center gap-1 sm:gap-2">
            <!-- Hamburger móvil -->
            <button type="button" class="btn rounded-xl p-2" aria-label="Abrir menú"
                @click="$emit('toggle-sidebar')">
                <Icon icon="heroicons:bars-3" class="h-5 w-5 lg:hidden" aria-hidden="true" />
                <!-- Desktop: toggle collapse -->

                <Icon v-if="sidebarHidden" icon="heroicons:bars-3" class="hidden h-5 w-5 lg:block" aria-hidden="true"
        ...
        <div
            class="relative hidden min-w-0 flex-1 sm:flex sm:max-w-xs md:max-w-sm lg:max-w-xl xl:max-w-2xl mx-2 sm:mx-4">
            <form action="#" method="GET" class="relative w-full">
                <label for="search-field" class="sr-only">Buscar</label>
                <Icon icon="heroicons:magnifying-glass"
                    class="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2"
                    style="color: var(--th-text-muted)" aria-hidden="true" />
                <input id="search-field" type="search" name="search" placeholder="Buscar..."
                    class="search-input block w-full rounded-xl py-2 pl-9 pr-14 text-sm placeholder:italic transition focus:ring-0 sm:py-2.5" />
                <div class="pointer-events-none absolute right-3 top-1/2 -translate-y-1/2 flex items-center">
                    <kbd class="kbd rounded border px-1.5 py-0.5 text-[10px] font-medium">⌘K</kbd>
                </div>
            </form>
        </div>
        ...
        <div class="flex shrink-0 items-center gap-0.5 sm:gap-1">
            ...
            <button type="button" class="btn rounded-xl p-2 sm:hidden" aria-label="Buscar"
                @click="openMobileSearch">
                <Icon icon="heroicons:magnifying-glass" class="h-5 w-5" aria-hidden="true" />
            </button>

            <!-- Toggle tema -->
            <button type="button" class="btn rounded-xl p-2" :title="themeLabel" :aria-label="themeLabel"
                @click="cycleTheme">
                <Icon :icon="themeIcon" class="h-5 w-5" aria-hidden="true" />
            </button>

            <!-- Notificaciones -->
            <button type="button" class="btn relative rounded-xl p-2" aria-label="Notificaciones">
                <Icon icon="heroicons:bell" class="h-5 w-5" aria-hidden="true" />
                <span class="notification-dot absolute right-2 top-2 h-2 w-2 animate-pulse rounded-full ring-2"
                    style="--tw-ring-color: var(--th-ring-offset)" aria-hidden="true" />
            </button>

            <div class="separator mx-0.5 h-5 w-px sm:mx-1" aria-hidden="true" />
                ...
                <button type="button"
                    ...
                    <div class="relative shrink-0">
                        ...
                        <span
                            ...
                            :style="`--tw-ring-color: var(--th-status-ring)`" aria-hidden="true" />
                    </div>
                    <span class="hidden min-w-0 text-left lg:block">
                        <span class="user-name block truncate text-sm font-medium">{{ user?.name || 'Usuario'
                            }}</span>
                        <span class="user-sub block truncate text-xs">Admin</span>
                    </span>
                    <Icon icon="heroicons:chevron-down" class="h-3.5 w-3.5 shrink-0 hidden sm:block"
                        style="color: var(--th-text-muted)" aria-hidden="true" />
                </button>
