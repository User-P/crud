<template>
    ...
    <div v-if="mobileSearchOpen" class="cosmos-nav fixed inset-x-0 top-0 z-50 flex h-14 items-center gap-3 px-4">
        ...
        <button type="button" class="cosmos-btn rounded-xl p-2" aria-label="Cerrar búsqueda"
    ...
    <header
        ...
        <div class="flex min-w-0 shrink-0 items-center gap-1 sm:gap-2">
            <!-- Hamburger móvil -->
            <button type="button" class="cosmos-btn rounded-xl p-2" aria-label="Abrir menú"
        ...
        <div class="flex shrink-0 items-center gap-0.5 sm:gap-1">
            ...
            <button type="button" class="cosmos-btn rounded-xl p-2 sm:hidden" aria-label="Buscar"
                @click="openMobileSearch">
                <Icon icon="heroicons:magnifying-glass" class="h-5 w-5" aria-hidden="true" />
            </button>

            <!-- Toggle tema -->
            <button type="button" class="cosmos-btn rounded-xl p-2" :title="themeLabel" :aria-label="themeLabel"
                @click="cycleTheme">
                <Icon :icon="themeIcon" class="h-5 w-5" aria-hidden="true" />
            </button>

            <!-- Notificaciones -->
            <button type="button" class="cosmos-btn relative rounded-xl p-2" aria-label="Notificaciones">
                <Icon icon="heroicons:bell" class="h-5 w-5" aria-hidden="true" />
                <span class="cosmos-notification-dot absolute right-2 top-2 h-2 w-2 animate-pulse rounded-full ring-2"
            ...
            <div class="relative">
                <button type="button"
                    ...
                    <span class="hidden min-w-0 text-left lg:block">
                        <span class="cosmos-user-name block truncate text-sm font-medium">{{ user?.name || 'Usuario'
                            }}</span>
