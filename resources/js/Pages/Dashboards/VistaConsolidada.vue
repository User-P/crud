    ...
    <AdminLayout title="Dashboard de métricas" subtitle="Vista consolidada: todos los indicadores en una sola pantalla"
        ...
        <div class="space-y-14">
            ...
            <section :id="sectionIds.vistaGeneral" class="scroll-mt-6">
                <div class="consolidated-section rounded-r-2xl border border-[var(--th-border)] border-l-4 border-l-[#0b4261] bg-[var(--th-input-bg)] pl-6 pr-5 py-5 dark:border-l-[#5bb56a] space-y-6">
                    <h2 class="mb-6 text-lg font-semibold text-[color:var(--th-text-primary)] pb-2">
                        Vista general
                    </h2>
                    ...
                    <div>
                        <p class="mb-4 text-xs font-semibold uppercase tracking-widest text-[color:var(--th-group-label)]">Otras
                            métricas</p>
                        <div
                            class="flex flex-col overflow-hidden rounded-2xl border border-[var(--th-border)] bg-[var(--th-input-bg)] shadow-sm">
                            <button v-for="card in secondaryCards" :key="card.id" type="button"
                                class="group flex w-full items-center gap-4 border-b border-[var(--th-border)] px-5 py-4 text-left last:border-b-0 transition-colors hover:bg-[var(--th-item-hover-bg)] focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[var(--p-focus-ring-color)]"
                                :class="{ 'bg-[var(--th-item-hover-bg)]': selectedCard?.id === card.id }"
                                @click="selectedCard = card">
                                ...
                                <span class="min-w-0 flex-1 font-medium text-[color:var(--th-text-primary)]">{{ card.label
                                    }}</span>
                                <span class="tabular-nums font-semibold text-[color:var(--th-text-primary)]">{{ card.value
                                    }}</span>
                                <Icon icon="heroicons:chevron-right"
                                    class="h-5 w-5 shrink-0 text-[color:var(--th-text-muted)] group-hover:text-[color:var(--th-item-active-color)]"
                                    aria-hidden="true" />
                            </button>
                        </div>
                    </div>
                    <div class="rounded-2xl border border-[var(--th-border)] bg-[var(--th-input-bg)] overflow-hidden shadow-sm">
                        <div
                            class="flex flex-wrap items-center justify-between gap-3 border-b border-[var(--th-border)] px-4 py-3">
                            ...
                            <div class="flex items-center gap-2">
                                <button v-if="useTestData1M" type="button"
                                    class="text-xs font-medium text-[color:var(--th-text-muted)] hover:text-[color:var(--th-item-active-color)] transition-colors"
                                ...
                                <span v-else-if="loadingTestData" class="text-xs text-[color:var(--th-text-muted)]">
                        ...
                        <div v-if="!selectedCard && !useTestData1M"
                            ...
                            <Icon icon="heroicons:table-cells" class="h-10 w-10 text-[color:var(--th-text-muted)]"
                                aria-hidden="true" />
                            <p class="text-sm text-[color:var(--th-text-muted)]">Selecciona una métrica arriba para ver el detalle.
                            </p>
                            <p class="text-xs text-[color:var(--th-text-muted)]">O usa «Cargar 1M registros de prueba» para pruebas de rendimiento y exportación.</p>
                        </div>
                        <div v-else-if="loadingTestData" class="flex flex-col items-center justify-center gap-2 px-4 py-12 text-center">
                            ...
                            <p class="text-sm text-[color:var(--th-text-muted)]">Generando 1.000.000 de filas con 10 columnas…</p>
                            <p class="text-xs text-[color:var(--th-text-muted)]">Puede tardar unos segundos.</p>
                        </div>
            ...
            <section :id="sectionIds.activosInactivos" class="scroll-mt-6">
                <div class="consolidated-section rounded-r-2xl border border-[var(--th-border)] border-l-4 border-l-emerald-500 bg-[var(--th-input-bg)] pl-6 pr-5 py-5 dark:border-l-emerald-400">
                    <h2 class="mb-6 text-lg font-semibold text-[color:var(--th-text-primary)] pb-2">
                        Usuarios activos vs inactivos
                    </h2>
                <div class="space-y-8">
                    <div
                        class="rounded-xl border border-[var(--th-border)] bg-[var(--th-input-bg)] px-4 py-3 text-sm text-[color:var(--th-text-secondary)] backdrop-blur-sm">
                        <span class="font-medium text-[color:var(--th-text-primary)]">Resumen:</span>
            ...
            <section :id="sectionIds.diasSuspendidos" class="scroll-mt-6">
                <div class="consolidated-section rounded-r-2xl border border-[var(--th-border)] border-l-4 border-l-amber-500 bg-[var(--th-input-bg)] pl-6 pr-5 py-5 dark:border-l-amber-400">
                    <h2 class="mb-6 text-lg font-semibold text-[color:var(--th-text-primary)] pb-2">
                ...
                <div class="space-y-8">
                    <div
                        class="rounded-xl border border-[var(--th-border)] bg-[var(--th-input-bg)] px-4 py-3 text-sm text-[color:var(--th-text-secondary)] backdrop-blur-sm">
                        <span class="font-medium text-[color:var(--th-text-primary)]">Insight:</span>
                        El 44% de los usuarios suspendidos llevan 7+ días (riesgo elevado). Priorizar revisión de ese
                        grupo.
                    </div>
                    <div
                        class="flex flex-wrap gap-6 rounded-xl border border-[var(--th-border)] bg-[var(--th-input-bg)] px-5 py-4">
                        <div class="flex items-center gap-2">
                            <span class="h-3 w-3 rounded-full bg-amber-400" aria-hidden="true" />
                            <span class="text-sm text-[color:var(--th-text-secondary)]">1-3 días (riesgo menor)</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="h-3 w-3 rounded-full bg-orange-500" aria-hidden="true" />
                            <span class="text-sm text-[color:var(--th-text-secondary)]">4-6 días (riesgo moderado)</span>
                        </div>
                        <div class="flex items-center gap-2">
                            <span class="h-3 w-3 rounded-full bg-rose-500" aria-hidden="true" />
                            <span class="text-sm text-[color:var(--th-text-secondary)]">7+ días (riesgo elevado)</span>
                        </div>
                    </div>
            ...
            <section :id="sectionIds.usuariosNuevos" class="scroll-mt-6">
                <div class="consolidated-section rounded-r-2xl border border-[var(--th-border)] border-l-4 border-l-blue-500 bg-[var(--th-input-bg)] pl-6 pr-5 py-5 dark:border-l-blue-400">
                    <h2 class="mb-6 text-lg font-semibold text-[color:var(--th-text-primary)] pb-2">
                        Usuarios nuevos
                    </h2>
                <div class="space-y-8">
            ...
            <section :id="sectionIds.unidades" class="scroll-mt-6">
                <div class="consolidated-section rounded-r-2xl border border-[var(--th-border)] border-l-4 border-l-[#0b4261] bg-[var(--th-input-bg)] pl-6 pr-5 py-5 dark:border-l-[#5bb56a]">
                    <h2 class="mb-5 text-xs font-semibold uppercase tracking-widest text-[color:var(--th-group-label)]">
                        Por unidad de negocio
                    </h2>
...
const detailTableColumnsTest: DetailMetricColumn[] = [
    ...
    { key: 'fecha', header: 'Fecha', sortable: true, class: 'text-[color:var(--th-text-secondary)]' },
    ...
    { key: 'observaciones', header: 'Observaciones', sortable: true, class: 'text-[color:var(--th-text-secondary)]' },
]
